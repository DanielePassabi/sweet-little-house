...
assert.equal(canWalk(3.5,13.72),true,'Free space beside dining table must not be blocked');
assert.equal(canWalk(6.15,13),false,'Round table remains solid');
assert.equal(canWalk(4.36,13.72),true,'Old dining furniture removed');
...
assert.equal(byId('window-return').d,.45);
